...
impl Id {
    /// Creates a BitShares asset id from its object instance component.
    ...
    /// Returns the asset object instance component.
...
/// Error returned when parsing or converting a BitShares asset id fails.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum Error {
    /// The underlying object id string could not be parsed.
    Parse(ObjectIdParseError),
    /// The object id is not in the BitShares asset object range (`1.3.x`).
    WrongType {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::WrongType { actual } => {
                write!(f, "object id {actual} is not a BitShares asset id")
            }
        }
    }
}
