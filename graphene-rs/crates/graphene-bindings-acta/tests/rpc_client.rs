use graphene_bindings_acta::{lookup_exact_account_id, GetAccountCountParams};
...
impl RpcTransport for AccountLookupTransport {
    fn call_raw(&self, method: &str, params: Vec<Value>) -> Result<Value, RpcError> {
        ...
        assert_eq!(params, vec![json!("acta"), json!(1), json!(false)]);
        Ok(json!([["acta", "1.2.100"]]))
    }
}
...
fn lookup_exact_account_id_uses_lookup_accounts_with_exact_match() {
    ...
    let account_id = lookup_exact_account_id(&client, "acta").unwrap();
