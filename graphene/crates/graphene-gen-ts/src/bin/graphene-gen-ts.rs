...
fn usage() -> String {
    "usage: graphene-gen-ts generate <opengraphene-path> [--openrpc <openrpc-path>] --out <dir>"
        .to_owned()
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let files = emit_typescript(&ir)?;
    let written = write_generated_files(&args.out, &files)?;

    println!("generated TypeScript target into {}", args.out.display());
    for file in written {
