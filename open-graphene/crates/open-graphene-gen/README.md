...
`open-graphene-gen` is the OpenGraphene contract compiler for the Graphene SDK workspace. It gives future SDK and emitter work one stable entrypoint for validating a chain contract, inspecting the lowered intermediate representation (IR), generating reviewable TypeScript and Dart metadata samples, and emitting byte-level conformance fixtures.
...
- `src/bin/open-graphene-gen.rs` is the CLI entrypoint and should remain the documented operator surface.
...
```text
crates/open-graphene-gen/fixtures/generated-samples/typescript/index.ts
crates/open-graphene-gen/fixtures/generated-samples/dart/lib/open_graphene.dart
```
