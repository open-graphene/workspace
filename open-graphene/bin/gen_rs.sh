...
ROOT_OUT_DIR="${REPO_DIR}/open-graphene-rs"
...
for CHAIN in swaplock acta; do
  ...
  OUT_DIR="${ROOT_OUT_DIR}/open-graphene-rs-bindings-${CHAIN}"
  ...
  (
    ...
    cargo run -p open-graphene-gen -- generate \
      ...
      --openrpc "crates/open-graphene-gen/fixtures/${CHAIN}.openrpc.json" \
      --target rust \
      --out "${TMP_CHAIN_DIR}"
  )
...
[package]
name = "open-graphene-rs-bindings-${CHAIN}"
edition = "2021"
version = "0.1.0"
publish = false
...
EOF
  rm -rf "${TMP_CHAIN_DIR}"
  echo "regenerated Rust OpenGraphene SDK artifacts in ${OUT_DIR}"
done
