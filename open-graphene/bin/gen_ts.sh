...
ROOT_OUT_DIR="${REPO_DIR}/open-graphene-ts"
...
for CHAIN in swaplock acta; do
  ...
  OUT_DIR="${ROOT_OUT_DIR}/open-graphene-ts-bindings-${CHAIN}"
  ...
  rm -rf "${TMP_CHAIN_DIR}"
  echo "regenerated TypeScript OpenGraphene SDK artifacts in ${OUT_DIR}"
done
